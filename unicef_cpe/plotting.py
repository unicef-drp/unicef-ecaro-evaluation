...
# Uniform Color pallete for SDG implementing and Contributing partners
SDG_partners = ['CIVIL SOCIETY ORGANIZATION', 'UN AGENCY', 'GOVERNMENT', 'MULTI-LATERAL ORGANIZATION','uncertain', 
                 'Multiple (excl. GOVERNMENT)', 'PRIVATE SECTOR', 'Multiple (incl. GOVERNMENT)' ]

SDG_partners_color = {t: c for t, c in zip(SDG_partners, PALLETE_QUALITATIVE)}
...
def plot_SDG_funds(df: pd.DataFrame):
    ...
    for i in range(1, 3):  # Assuming you have 2 rows
        fig.update_xaxes(range=[min_value, max_value], row=i, col=1, gridcolor='lightgrey')
...
def plot_SDG_agencies(df: pd.DataFrame):
    ...
    fig.update_layout(
        ...
        legend=legend_style,
        legend_title_text="",
        xaxis_title='Porportion of Total Expenses',
        yaxis_title=''
    )
...
def plot_SDG_partners(df:pd.DataFrame, partner_type, title):

    # aggregate Funds by SDG Area, Country and CP
    df = df.groupby(['agencies', partner_type], as_index=False)['total_expenditure'].sum().copy()
    # drop expenditures ~ 0
    df = df[abs(df['total_expenditure']) > 0.1]
    df['proportion'] = df['total_expenditure'] / df.groupby('agencies')['total_expenditure'].transform('sum')
    df.sort_values(by=[partner_type], inplace=True, ascending=[True])
    # Create an ordering for agencies based on proportion of Expenses implemented/contributed by Government
    # 1. Government
    # 2. all others

    agency_list = list(df['agencies'].unique())

    df_gov = df.loc[df[partner_type] == 'GOVERNMENT'].copy()
    df_gov.sort_values(by=['proportion'], inplace=True, ascending=[False])
    gov_agency_list = list(df_gov['agencies'].unique())
 
    for agency in agency_list:
        # Append the agency to gov_agency_list if it's not already present
        if agency not in gov_agency_list:
            gov_agency_list.append(agency)

    partner_list = ['GOVERNMENT', 'Multiple (incl. GOVERNMENT)', 'MULTI-LATERAL ORGANIZATION','CIVIL SOCIETY ORGANIZATION',
                     'UN AGENCY', 'Multiple (excl. GOVERNMENT)', 'PRIVATE SECTOR', 'uncertain', ]  
      
    # Convert the 'agency' column to a categorical type with the custom order

    df['agencies'] = pd.Categorical(df['agencies'], categories=gov_agency_list, ordered=True)
    # Convert the 'partners' column to a categorical type with the custom order
    df[partner_type] = pd.Categorical(df[partner_type], categories=partner_list, ordered=True)

    # Sort the DataFrame by 'agencies' and then by 'partners'
    df = df.sort_values(by=['agencies', partner_type])
    # Sort the DataFrame by the categorical order
    #df = df.sort_values(by='agencies')
    ...
    fig = px.bar(
        ...
        color_discrete_map=SDG_partners_color
    )
    ...
    fig.update_layout(
        ...
        legend_title_text='',
        yaxis_title='',
        xaxis_title='Expenses Proportion',
        showlegend=True,
        legend=legend_style,
    )
    # fig.write_image(COUNTRY + '_contributing.png', format="png", engine="kaleido", scale=3)  
    return fig
